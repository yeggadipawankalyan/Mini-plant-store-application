...
async function updatePlantImagesUnique() {
    try {
        console.log('🖼️ Updating plant images with unique, specific images...');

        // Unique image URLs for each specific plant type from Unsplash
        const imageUpdates = [
            {
                name: 'Snake Plant',
...
if (require.main === module) {
    updatePlantImagesUnique()
        .then(() => {
            console.log('🎉 Unique image update completed successfully!');
            process.exit(0);
        })
        .catch((error) => {
            console.error('💥 Unique image update failed:', error);
            process.exit(1);
        });
}

module.exports = { updatePlantImagesUnique };
